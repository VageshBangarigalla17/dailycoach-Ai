...
- **Services:**
  ...
  - `voiceService.js`: Web Speech API implementation.
...
- **Key Modules:**
  ...
  - Reminder scheduler job (`reminderScheduler.js`).
  - Middleware for error handling and JWT validation.

===================================================
5. DATABASE DESIGN
===================================================
- **Users:** `email`, `passwordHash`, `name`, `preferredVoiceGender`, `timezone`.
- **Schedules:** `userId`, `taskName`, `startTime`, `endTime`, `daysOfWeek`, `color`.
...
{
  "version": 2,
  "installCommand": "npm install --legacy-peer-deps && cd server && npm install --legacy-peer-deps && cd ..",
...
### FILE: server\jobs\reminderScheduler.js
```javascript
...
const startScheduler = () => {
  ...
  cron.schedule('* * * * *', async () => {
    try {
      ...
      // 1. Check for START times (Send Push Notification)
      ...
      // 2. Check for END times (Send Push Notification + Auto-miss if no log exists)
      ...
      if (endSchedules.length > 0) {
        for (const schedule of endSchedules) {
          ...
          // We no longer auto-mark as missed at the exact end time.
          // The user has 30 seconds to respond, and if they don't, 
          // the frontend handles marking it as 'no-response'.
          // If the frontend is closed, the +10 min loop will handle the followups.
        }
      }
      ...
      for (const schedule of allSchedules) {
        ...
        if (currentMins > endMins && (currentMins - endMins) % 10 === 0) {
          ...
          if (!existingLog || existingLog.status === 'active' || existingLog.status === 'no-response') {
             ...
             // Note: We no longer auto-mark as missed here, so the loop continues until the user responds.
          }
        }
      }
...
const dailyLogSchema = new mongoose.Schema({
  ...
  reminderType: { type: String, enum: ['start', 'followup', 'second-chance'], default: 'start' },
...
### FILE: server\models\UserStats.js
```javascript
const mongoose = require('mongoose');
...
@layer base {
  :root {
    ...
    --color-afternoon: #F97316;
    --color-evening: #8B5CF6;
    --color-night-dark: #1E293B;
    --color-done: #22C55E;
    --color-late: #EAB308;
    --color-missed: #EF4444;
  }
...
@theme {
  ...
  --color-morning: var(--color-morning);
  --color-afternoon: var(--color-afternoon);
  --color-evening: var(--color-evening);
  --color-night-dark: var(--color-night-dark);
  --color-done: var(--color-done);
  --color-late: var(--color-late);
  --color-missed: var(--color-missed);
}
...
### FILE: src\components\ReminderModal.jsx
```javascript
    ...
    <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-background/95 backdrop-blur-md p-6">
          ...
          `}>
            ...
            {(isSpeaking || phase === 'speaking') && (
              <>
                ...
                <div className="absolute -inset-2 rounded-full border-2 border-morning/20 animate-pulse"></div>
              </>
            )}
            ...
            {phase === 'listening' && (
              <div className="flex flex-col items-center">
                <Mic size={36} className="text-done animate-bounce" />
                <span className="text-done text-lg font-bold font-mono mt-1">{listenCountdown}s</span>
              </div>
            )}
            {phase === 'responding' && (
      ...
      {phase !== 'done' && phase !== 'generating' && phase !== 'waiting' && (
        <div className="w-full max-w-md mt-8 grid grid-cols-2 gap-4">
          ...
          >
            ...
            {reminderType === 'start' ? "OK, I'LL START" : "YES, DONE"}
          </button>
          <button
      ...
      {phase === 'done' && (
        <div className="w-full max-w-md mt-8">
          <button
            onClick={handleDismiss}
            className="w-full py-4 bg-slate-800/60 hover:bg-slate-700/60 border border-slate-600 rounded-xl text-slate-300 font-medium font-sans transition-all"
          >
            Close Now
          </button>
        </div>
...
### FILE: src\contexts\ScheduleContext.jsx
```javascript
import React, { createContext, useContext, useState, useEffect } from 'react';
...
export default function Dashboard() {
  ...
  // ----------------------------------------------------------------
  // 1-SECOND polling — check for exact start/end triggers
  // ----------------------------------------------------------------
  useEffect(() => {
    const checkReminders = () => {
      ...
      for (const task of todayTasks) {
        const taskStartSecs = timeToSeconds(task.startTime);
        const taskEndSecs = timeToSeconds(task.endTime);
        const taskId = task.id || task._id;
        const log = todayLogs[taskId];

        const isAlreadyCompleted = log && (log.status === 'done' || log.status === 'late' || log.status === 'missed');
        const hasStartLog = !!log;

        // --- START TRIGGER: fire within ±2 seconds of start time ---
        const startKey = `start-${taskId}`;
        if (
          Math.abs(currentTotalSecs - taskStartSecs) <= 2 &&
          !hasStartLog &&
          !firedRemindersRef.current.has(startKey)
        ) {
          console.log(`[Dashboard] ⏰ START trigger: ${task.taskName}`);
        ...
        // --- FOLLOW-UP TRIGGER: fire within ±2 seconds of end time ---
        // Do not fire if already done, late, or missed.
        // DO fire if there is no log, or if log is 'no-response'.
        const followupKey = `followup-${taskId}`;
        if (
          Math.abs(currentTotalSecs - taskEndSecs) <= 2 &&
          !isAlreadyCompleted &&
          !firedRemindersRef.current.has(followupKey)
        ) {
          console.log(`[Dashboard] ⏰ FOLLOW-UP trigger: ${task.taskName}`);
        ...
        // --- SECOND CHANCE TRIGGER: Loop every 10 mins (600s) past end time ---
        if (currentTotalSecs > taskEndSecs + 590) { // start checking near first 10m mark
          const diff = currentTotalSecs - taskEndSecs;
          const cycleCount = Math.floor(diff / 600);
          const secondChanceKey = `secondchance-${taskId}-${cycleCount}`;
          
          if (
            (diff % 600) <= 2 &&
            (!log || log.status === 'active' || log.status === 'no-response') &&
            !firedRemindersRef.current.has(secondChanceKey)
          ) {
            console.log(`[Dashboard] ⏰ SECOND-CHANCE trigger (Loop ${cycleCount}): ${task.taskName}`);
            firedRemindersRef.current.add(secondChanceKey);
            setReminderType('second-chance');
            setActiveReminderTask(task);
            return;
          }
        }
  ...
  const handleReminderComplete = useCallback(async (status, voiceResponse) => {
    ...
    try {
      ...
      if (res.data.success) {
        console.log(`[Dashboard] Task "${task.taskName}" logged as ${status} (type: ${currentRemType})`);
        setTodayLogs(prev => ({
          ...prev,
          [taskId]: {
      ...
      setTodayLogs(prev => ({
        ...prev,
        [taskId]: { status, completedAt: status === 'missed' || status === 'no-response' ? null : new Date().toISOString() }
      }));
    }
...
export default function Reports() {
  ...
  const heatmapData = last7Days.map(date => {
    ...
    if (logsForDay.length === 0) {
      return date > new Date() ? 'upcoming' : 'slate-700';
    }
  ...
  const getColorForStatus = (status) => {
    switch (status) {
      case 'done': return 'bg-done shadow-[0_0_15px_rgba(34,197,94,0.3)]';
      case 'late': return 'bg-late shadow-[0_0_15px_rgba(234,179,8,0.3)]';
      case 'missed': return 'bg-missed shadow-[0_0_15px_rgba(239,68,68,0.3)]';
  ...
  if (loading) {
    return <div className="min-h-screen bg-background p-4 flex items-center justify-center text-white">Loading reports...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background p-4 max-w-md mx-auto">
      ...
      {/* Streak Card */}
      <div className="bg-gradient-to-br from-orange-500/20 to-red-500/20 border border-orange-500/30 rounded-2xl p-6 mb-6 flex items-center justify-between shadow-lg">
        <div>
          ...
          {stats.currentStreakDays > 0 && (
            <p className="text-xs text-orange-300 mt-1">You're doing great! Keep it up 🔥</p>
          )}
        </div>
      ...
      {/* Weekly Heatmap */}
      <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-5 border border-slate-700 mb-6 shadow-md">
        ...
        <div className="flex justify-between text-xs text-slate-500 mt-4 border-t border-slate-700 pt-3">
          <span className="flex items-center"><div className="w-2 h-2 rounded-full bg-done mr-1"></div> Perfect</span>
          <span className="flex items-center"><div className="w-2 h-2 rounded-full bg-late mr-1"></div> Partial</span>
      ...
      <div className="grid grid-cols-2 gap-4 mb-6">
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
          ...
          <p className="text-xs text-slate-400">Week Completion</p>
        </div>
        
        <div className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
          <div className="flex items-center justify-between mb-2">
            <CheckCircle size={18} className="text-morning" />
            <span className="text-2xl font-bold text-white">{stats.totalTasksCompleted}</span>
          </div>
      ...
      {/* Best & Worst Tasks */}
      <div className="bg-slate-800/50 backdrop-blur-md rounded-2xl p-5 border border-slate-700 mb-8 shadow-md">
        <h2 className="text-lg font-bold text-white mb-4">Insights</h2>
        <div className="flex justify-between items-center mb-3">
          <div className="flex items-center">
            <Trophy size={16} className="text-done mr-2" />
            <span className="text-sm text-slate-300">Best Task</span>
          </div>
          <span className="text-sm font-bold text-white">{stats.bestPerformedTask}</span>
        </div>
        <div className="flex justify-between items-center pt-3 border-t border-slate-700">
          <div className="flex items-center">
            <AlertTriangle size={16} className="text-missed mr-2" />
            <span className="text-sm text-slate-300">Needs Work</span>
          </div>
          <span className="text-sm font-bold text-white">{stats.mostMissedTask}</span>
        </div>
      </div>

      {/* Today's Breakdown */}
      <h2 className="text-xl font-bold text-slate-200 mb-4 font-sans mt-8">Today's Breakdown</h2>
      <div className="space-y-3 mb-8">
        {todayLogs.length === 0 ? (
          <p className="text-slate-500 text-center text-sm italic">No tasks completed or missed today yet.</p>
        ) : (
          todayLogs.map((log, i) => (
            <div key={i} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/50">
              <div className="flex items-center">
                {log.status === 'done' && <CheckCircle size={16} className="text-done mr-2" />}
                {log.status === 'late' && <Clock size={16} className="text-late mr-2" />}
                {log.status === 'missed' && <XCircle size={16} className="text-missed mr-2" />}
                <span className="font-medium text-slate-300">{log.taskName}</span>
              </div>
              <span className={`text-xs px-2 py-1 rounded font-mono ${
                log.status === 'done' ? 'bg-done/20 text-done' :
                log.status === 'late' ? 'bg-late/20 text-late' : 'bg-missed/20 text-missed'
              }`}>
      ...
      <h2 className="text-xl font-bold text-slate-200 mb-4 font-sans">Task Performance (7 Days)</h2>
      <div className="space-y-3 mb-10">
        {taskRates.map((schedule, i) => (
          <div key={schedule.id || i} className="flex items-center justify-between p-3 bg-slate-800/30 rounded-lg border border-slate-700/50">
            <div className="flex flex-col flex-1 pr-4">
              <span className="font-medium text-slate-300 truncate mb-1">{schedule.taskName}</span>
              <div className="w-full bg-slate-700 h-2 rounded-full overflow-hidden">
                <div 
                  className={`h-full ${schedule.rate >= 80 ? 'bg-done' : schedule.rate >= 50 ? 'bg-late' : 'bg-missed'}`} 
                  style={{ width: `${schedule.rate}%` }}
                ></div>
              </div>
            </div>
            <div className="flex flex-col items-end min-w-[40px]">
              <span className="text-sm font-bold text-white">{Math.round(schedule.rate)}%</span>
              <span className="text-[10px] text-slate-500">{schedule.totalAttempts} attempts</span>
            </div>
          </div>
        ))}
      </div>
...
### FILE: src\pages\ScheduleBuilder.jsx
```javascript
import React, { useState, useEffect } from 'react';
import api from '../services/api';
